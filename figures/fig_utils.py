...
def predict_consumption(country, country_path, dimension=None, k=5, k_inner=5, points=10, alpha_low=1, alpha_high=5, margin=0.25):
  """
...
def plot_predictions(country, y, y_hat, r2, margin):
  ...
  """
  slope, intercept, ymin, ymax, xmin, xmax = compute_plot_params(y, y_hat, margin)
  sns.set_style('white')
  plt.figure()
  plt.axis('equal')
  ...
  plt.plot(x_trend, y_trend, 'b-', linewidth=2, color=sns.xkcd_rgb['french blue'])
  plt.xlim(xmin, xmax)
  plt.ylim(ymin, ymax)
...
def compare_models(country_path, survey, percentiles, dimension, k, k_inner, trials, poverty_line, multiples):
  """
  Evaluates and plots comparison of transfer learning and nightlight models.
  """
  r2s, r2s_nl = evaluate_percentiles(country_path, survey, percentiles, dimension, k, k_inner, trials)
  if survey == 'lsms':
    X, X_nl, y = load_and_reduce_country_by_percentile(country_path, survey, 1.0, dimension)
...
def load_and_reduce_country_by_percentile(country_path, survey, percentile, dimension):
  """
...
def evaluate_percentiles(country_path, survey, percentiles, dimension, k, k_inner, trials):
  """
  ...
  for idx, percentile in enumerate(percentiles):
    for trial in xrange(trials):
      X, X_nl, y = load_and_reduce_country_by_percentile(country_path, survey, percentile, dimension)
      _, r2 = run_cv(X, y, k, k_inner, points=10, alpha_low=0, alpha_high=3, randomize=False)
...
def compute_fractions(poverty_line, multiples, y):
  ...
  for idx, multiple in enumerate(multiples):
    fractions[idx] = (np.exp(y) <= poverty_line * multiple).sum() / float(y.size)
  return fractions
...
def plot_percentiles_lsms(percentiles, multiples, r2s, r2s_nl, fractions):
  ...
  for idx, multiple in enumerate(multiples):
    lines.append(plt.axvline(100*fractions[idx], color='r', linestyle='dashed', linewidth=3.0/(idx+1), label=str(multiple)+'x poverty line'))
  line_legend = plt.legend(handles=lines, title='Poverty line multiples:', loc='upper right', bbox_to_anchor=(0.5, 1), fontsize=10)
  ...
  plt.legend(handles=[curve1, curve2], loc='upper right', bbox_to_anchor=(0.5, 0.65))
...
def run_randomization_test(country_names, country_paths, survey, dimension, k, k_inner, points, alpha_low, alpha_high, trials):
  """
  Runs randomization test for a set of countries.
  """
  data = load_data(country_paths, survey, dimension)
  true_r2s = compute_true_r2s(data, k, k_inner, points, alpha_low, alpha_high)
  shuffled_r2s = compute_shuffled_r2s(data, k, k_inner, points, alpha_low, alpha_high, trials)
...
def compute_shuffled_r2s(data, k, k_inner, points, alpha_low, alpha_high, trials):
  """
  ...
  for data_idx, (X, y) in enumerate(data):
    for trial in range(trials):
      _, shuffled_r2s[data_idx, trial] = run_cv(X, y, k, k_inner, points, alpha_low, alpha_high, randomize=True)
  return shuffled_r2s
...
def plot_shuffled_distributions(country_names, survey, shuffled_r2s, true_r2s):
  ...
  for i in xrange(len(country_names)):
    plt.axvline(true_r2s[i], color=colors[i], linestyle='dashed', linewidth=2, label=country_names[i].capitalize() + ': $r^2={0:.2f}$'.format(true_r2s[i]))
  plt.legend(title='True {} Models:'.format(survey.upper()), loc='lower right', bbox_to_anchor=(0.6, 0.45), fontsize=12)
  for i in xrange(len(country_names)):
...
def evaluate_models(country_names, country_paths, survey, dimension, k, trials, points, alpha_low, alpha_high, cmap='Greens'):
  """
  Evaluates in- and out-of-country performance for models trained on set of countries given.
  """
  n = len(country_names)
  ...
  for trial in xrange(trials):
    for in_idx in xrange(n):
      performance_matrix[trial, :, in_idx] = compute_column(country_names[in_idx], data, in_idx, dimension, k, points, alpha_low, alpha_high)
...
def compute_column(country_name, data, in_idx, dimension, k, points, alpha_low, alpha_high):
  """
  Evaluates in- and out-of-country performance for model trained on one country.
  """
  in_data = data[in_idx]
  out_data = list(data)
  _ = out_data.pop(in_idx)
  if country_name == 'pooled':
    r2s_in, r2s_out = evaluate_pooled_r2s(out_data, dimension, k, points, alpha_low, alpha_high)
  else:
    r2s_in, r2s_out = evaluate_model_r2s(in_data, out_data, dimension, k, points, alpha_low, alpha_high)
...
def evaluate_pooled_r2s(all_data, dimension, k, points, alpha_low, alpha_high):
  ...
  for i in xrange(len(X_all_full)):
    kf.append(list(cross_validation.KFold(n=all_data[i][1].size, n_folds=k, shuffle=True)))
  for fold in xrange(k):
    X_train, X_test, y_train, y_test = split_pooled_data(kf, fold, X_all_full, y_all)
    X_train, X_test, X_all = reduce_and_scale_features(X_train, X_test, X_all_full, dimension)
    r2s_in, r2s_out = evaluate_alphas(X_train, X_test, X_all, y_train, y_test, y_all, r2s_in, r2s_out, fold, alphas)
...
def evaluate_model_r2s(in_data, out_data, dimension, k, points, alpha_low, alpha_high):
  """
  ...
  for train_ind, test_ind in kf:
    ...
    X_train, X_test, X_out = reduce_and_scale_features(X_train, X_test, X_out_full, dimension)
    r2s_in, r2s_out = evaluate_alphas(X_train, X_test, X_out, y_train, y_test, y_out, r2s_in, r2s_out, fold, alphas)
    fold += 1
  return r2s_in.mean(axis=0), r2s_out.mean(axis=1)


def evaluate_alphas(X_train, X_test, X_out, y_train, y_test, y_out, r2s_in, r2s_out, fold, alphas):
  """
...
def plot_circles(data, ax, **kwargs):
  ...
  h = np.abs(M).ravel()
  a = 0
  circles = EllipseCollection(widths=w, heights=h, angles=a, units='x', offsets=xy, transOffset=ax.transData, array=M.ravel(), **kwargs)
...
def run_cv(X, y, k, k_inner, points, alpha_low, alpha_high, randomize=False):
  ...
  fold = 0
  for train_idx, test_idx in kf:
    r2s, y_hat, fold = evaluate_fold(X, y, train_idx, test_idx, k_inner, alphas, r2s, y_hat, fold, randomize)
...
def find_best_alpha(X, y, k_inner, alphas):
  ...
  for idx, alpha in enumerate(alphas):
    y_hat = np.zeros_like(y)
    for train_idx, test_idx in kf:
      y_hat = predict_inner_test_fold(X, y, y_hat, train_idx, test_idx, alpha)
    r2 = stats.pearsonr(y, y_hat)[0] ** 2
    if r2 > best_r2:
      best_alpha = alpha
      best_r2 = r2
...
def evaluate_fold(X, y, train_idx, test_idx, k_inner, alphas, r2s, y_hat, fold, randomize):
  """
